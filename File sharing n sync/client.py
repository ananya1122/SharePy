...
class Client(basic.LineReceiver):
    ...
    def _display_response(self, lines = None):
        ...
        if str(lines).startswith('recv') :
            self.factory.deferred.addCallback(self._send_ack)
            return lines
    ...
    def _send_ack(self,message = None) :
        self.setLineMode()
        print 'lol'
        print message,'---'
...
class FileTransferProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        if line == 'ENDMSG':
            print 'end mssg recieved'
            #delay = reactor.callLater(2,self.buffer,self.factory.deferred)
    ...
    def rawDataReceived(self, data):
        ...
        if data.endswith('\r\n'):
            ...
            if validate_file_md5_hash(file_path, self.file_data[1]):
                ...
                print filename
                self.factory.deferred.callback('recv %s '%(filename))
            else:
                os.unlink(file_path)
...
if __name__ == '__main__':
    ...
    while(not os.path.isdir(options.path)):
        print "# Please enter valid path "
        options.path = raw_input()
    

    print 'Client started, incoming files will be saved to %s' % (options.path)
    
    stdio.StandardIO(Client(options.ip_address, options.port, options.path))
    reactor.run()    